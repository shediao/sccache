...
/// The default ccache server port.
...
fn ignore_all_server_io_errors() -> bool {
    match env::var("CCACHE_IGNORE_SERVER_IO_ERROR") {
...
fn run_server_process(startup_timeout: Option<Duration>) -> Result<ServerStartup> {
    ...
    let tempdir = tempfile::Builder::new().prefix("ccache").tempdir()?;
    ...
    let _child = process::Command::new(&exe_path)
        .current_dir(workdir)
        .env("CCACHE_START_SERVER", "1")
        .env("CCACHE_STARTUP_NOTIFY", &socket_path)
...
fn create_error_log() -> Result<File> {
    ...
    let name = match env::var("CCACHE_ERROR_LOG") {
        Ok(filename) if !filename.is_empty() => filename,
        _ => {
            bail!("Cannot read variable 'CCACHE_ERROR_LOG'");
        }
    };
...
/// If `CCACHE_ERROR_LOG` is set, redirect stderr to it.
...
fn run_server_process(startup_timeout: Option<Duration>) -> Result<ServerStartup> {
    ...
    let mut envp = {
        let mut v = vec![];
        let extra_vars = vec![
            (OsString::from("CCACHE_START_SERVER"), OsString::from("1")),
            (
                OsString::from("CCACHE_STARTUP_NOTIFY"),
...
/// Attempt to connect to an ccache server listening on `port`, or start one if no server is running.
            ...
            match run_server_process(startup_timeout)? {
                ServerStartup::Ok { port: actualport } => {
                    if port != actualport {
                        ...
                        bail!(
                            "ccache: Listening on port {} instead of {}",
                            actualport,
                            port
                        );
                    }
...
fn handle_compile_finished(
    ...
    if let Some(ret) = response.retcode {
        ...
        println!("ccache: Compiler killed by signal {}", signal);
        Ok(-2)
    } else {
        println!("ccache: Missing compiler exit status!");
        Ok(-3)
    }
}
    ...
    T: CommandCreatorSync,
{
                ...
                Err(e) => {
                    match e.downcast_ref::<io::Error>() {
                        Some(io_e) if io_e.kind() == io::ErrorKind::UnexpectedEof => {
                            eprintln!(
                                "ccache: warning: The server looks like it shut down \
                        ...
                        _ => {
                            ...
                            if ignore_all_server_io_errors() {
                                eprintln!(
                                    "ccache: warning: error reading compile response from server \
    ...
    Ok(status.code().unwrap_or_else(|| {
        if let Some(sig) = status_signal(status) {
            println!("ccache: Compile terminated by signal {}", sig);
        }
        // Arbitrary.
        2
    }))
...
/// Send a `Compile` request to the ccache server `conn`, and handle the response.
///
...
pub fn run_command(cmd: Command) -> Result<i32> {
    ...
    match cmd {
        ...
        Command::InternalStartServer => {
            ...
            if env::var("CCACHE_ERROR_LOG").is_ok() {
        ...
        Command::StopServer => {
            ...
            println!("Stopping ccache server...");
